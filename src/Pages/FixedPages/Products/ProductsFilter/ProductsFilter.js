...
const ProductsFilter = () => {
  ...
  const allproducts = Array.isArray(allproductes) ? allproductes?.filter(book => book?.state === '' || book?.state === 'Available') : [];
  ...
  const getUniqueValues = (data, key) => [...new Set(
    Array.isArray(data) ? data?.map((item) => item[key])
  : []
)];
