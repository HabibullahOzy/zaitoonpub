...
const ClasslinkBasedShow = () => {
    ...
    const classicbooks = useLoaderData()

    // console.log(classicbooks)

    const queryClient = useQueryClient();

    const nursproduct = Array.isArray(classicbooks)
        ? classicbooks?.filter(book => book?.state === '' || book?.state === 'Available')
        : [];
...
export default ClasslinkBasedShow;