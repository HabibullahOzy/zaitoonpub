...
const ClassbooksShow = ({ productCategory }) => {
  ...
  const cartMatch = cartItems.map(c => c.ProductCode);

  return (
    <div>

      {/* FIX #2: correct heading */}
...
//         queryKey: ['classicproducts'],
//         queryFn: async () => {
//             const res = await fetch(`${process.env.REACT_APP_backendurl}/subcategoryproducts/${productCategory}`);
